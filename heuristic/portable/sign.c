...
{
  unsigned char c[CRYPTO_C_BYTES], randomness[CRYPTO_SEEDBYTES], randomness_input[CRYPTO_RANDOMBYTES+CRYPTO_SEEDBYTES+2*HM_BYTES], seeds[2*CRYPTO_SEEDBYTES], hash_pk[HM_BYTES];
  uint32_t pos_list[PARAM_H];
  int16_t sign_list[PARAM_H], s[PARAM_N], e[PARAM_N*PARAM_K];
  poly y, y_ntt, Sc, z; 
  poly_k v, Ec, a;
  ...
  decode_sk(seeds, s, e, hash_pk, sk);
  ...
  memcpy(&randomness_input[CRYPTO_RANDOMBYTES+CRYPTO_SEEDBYTES+HM_BYTES], hash_pk, HM_BYTES);
  ...
  rejyzctr++;
    ...
    for (k=0; k<PARAM_K; k++) {
      sparse_mul16(&Ec[k*PARAM_N], &e[k*PARAM_N], pos_list, sign_list);
